 ...
 *
 * One route serves all nine, statically generated at build and refreshed by ISR. The
...
export async function generateStaticParams() {
  const services = await content.solutions();
  return services.map((service) => ({ slug: service.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const [page, settings] = await Promise.all([content.solution(slug), content.settings()]);
  if (!page) return {};
    ...
    <>
      <BreadcrumbSchema
        trail={[
          ...
          { name: page.name, path: `/solutions/${slug}/` },
        ]}
      />
      <ServiceSchema
        name={page.name}
